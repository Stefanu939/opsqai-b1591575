...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'avgConfidence', COALESCE((
      SELECT round(avg(m.confidence)::numeric, 2)
      FROM messages m
...
BEGIN
  ...
  v_score := LEAST(100, v_docs * 5) * 0.20
  ...
  RETURN jsonb_build_object(
    ...
    'breakdown', jsonb_build_object(
      'documents', v_docs,
      'criticalSops', v_critical,
      'openGaps', v_gaps,
      'avgConfidence', round(v_conf, 2),
      'faqs', v_faqs,
      'lastAuditScore', round(v_audit_score)
    )
  );
END
$function$;
...
GRANT EXECUTE ON FUNCTION public.dashboard_health(uuid) TO authenticated, service_role;
...
BEGIN
  ...
  SELECT COALESCE((public.dashboard_health(p_company)->>'score')::int, 0) INTO v_workspace_health;

  SELECT jsonb_build_object(
    'workspaceHealth', v_workspace_health,
