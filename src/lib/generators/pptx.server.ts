...
export async function generatePptx(spec: PptxSpec): Promise<Uint8Array> {
  ...
  title.addText(spec.title, {
    x: 0.6, y: 2.2, w: 12, h: 1.5, fontSize: 44, bold: true, color: fg, fontFace: "Calibri",
  });
  if (spec.subtitle) {
    title.addText(spec.subtitle, {
      x: 0.6, y: 3.8, w: 12, h: 0.8, fontSize: 20, color: accent, fontFace: "Calibri",
    });
  }
  ...
  for (const slide of spec.slides) {
    ...
    if (slide.title) {
      s.addText(slide.title, {
        x: 0.6, y: 0.4, w: 12, h: 0.8, fontSize: 28, bold: true, color: fg, fontFace: "Calibri",
      });
    }
    if (slide.subtitle) {
      s.addText(slide.subtitle, {
        x: 0.6, y: 1.2, w: 12, h: 0.5, fontSize: 16, color: accent, italic: true,
      });
    }
