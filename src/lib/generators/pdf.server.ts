...
export async function generatePdf(spec: PdfSpec): Promise<Uint8Array> {
  ...
  const MUTED = rgb(0.55, 0.6, 0.69); // #8C99B0
  const BRAND = rgb(0.227, 0.357, 0.722); // #3A5BB8
  const BRAND_SOFT = rgb(0.416, 0.51, 0.784); // #6A82C8
  const TEAL = rgb(0.122, 0.58, 0.522); // #1F9485
  const AMBER = rgb(0.722, 0.525, 0.18); // #B8862E
  ...
  const PANEL_BG = rgb(0.969, 0.973, 0.98); // #F7F8FA
  ...
  cover.drawText(sanitize(meta.brand), {
    x: MARGIN_X + 12,
    y: PAGE_H - 82,
    size: 16,
    font: bold,
  ...
  // Metadata block (bottom, editorial)
  {
    ...
    const drawCol = (items: Array<[string, string]>, x: number) => {
      ...
      for (const [k, v] of items) {
        cover.drawText(sanitize(k.toUpperCase()), {
          x,
          y: yy,
          size: 7.5,
          font: bold,
  ...
  const drawParagraph = (
    text: string,
    opts: { font?: any; size?: number; color?: any; gap?: number; x?: number; width?: number } = {},
  ) => {
  ...
  const calloutTheme = (kind?: string) => {
    switch (kind) {
      case "recommendation":
        return { bar: BRAND, tag: "RECOMMENDATION", icon: "→" };
      case "risk":
        return { bar: RED, tag: "RISK", icon: "!" };
      case "opportunity":
        return { bar: TEAL, tag: "OPPORTUNITY", icon: "+" };
      case "key-takeaway":
        return { bar: BRAND_SOFT, tag: "KEY TAKEAWAY", icon: "*" };
      case "best-practice":
        return { bar: TEAL, tag: "BEST PRACTICE", icon: "*" };
      case "executive":
        return { bar: INK, tag: "EXECUTIVE NOTE", icon: "*" };
      default:
        return { bar: SUB, tag: "NOTE", icon: "i" };
    }
  };
  ...
  const drawCallout = (b: Extract<PdfBlock, { type: "callout" }>) => {
    ...
    yy -= tagH;
    if (b.title) {
      ctx.page.drawText(sanitize(b.title), {
        x: MARGIN_X + 18,
        y: yy,
        size: 12,
        font: bold,
