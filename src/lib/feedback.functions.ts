...
export const rateMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const companyId = (profile as { company_id: string | null; department_id: string | null } | null)?.company_id;
    const departmentId = (profile as { company_id: string | null; department_id: string | null } | null)?.department_id ?? null;
    if (!companyId) throw new Error("No company");
    const { error } = await context.supabase
      .from("message_feedback")
      .upsert({
        message_id: data.message_id, user_id: context.userId,
        company_id: companyId, rating: data.rating,
    ...
    if (data.rating === -1) {
      try {
        ...
        const asstRow = asst as { id: string; thread_id: string; confidence: number | null; created_at: string } | null;
        if (asstRow) {
          ...
          if (userMsg && userMsg.content?.trim().length > 4) {
            ...
            if (matched) {
              const { data: cur } = await context.supabase
                .from("knowledge_gaps").select("occurrences").eq("id", matched).maybeSingle();
              const occ = ((cur as { occurrences: number } | null)?.occurrences ?? 1) + 1;
              await context.supabase.from("knowledge_gaps")
                .update({ occurrences: occ, last_seen: new Date().toISOString(), status: "open" } as never)
                .eq("id", matched).in("status", ["open", "in_progress"]);
            } else {
      ...
      } catch (e) { console.error("[feedback:gap] failed", e); }
    }
    return { ok: true };
  });
