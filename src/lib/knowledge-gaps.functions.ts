...
export const listKnowledgeGaps = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const gaps = (data ?? []) as Array<Record<string, unknown> & {
      ...
      resolved_document_id: string | null; resolved_faq_id: string | null;
    }>;

    const deptIds = Array.from(new Set(gaps.map((g) => g.department_id).filter(Boolean) as string[]));
    const userIds = Array.from(new Set(gaps.map((g) => g.created_by).filter(Boolean) as string[]));
    const docIds  = Array.from(new Set(gaps.map((g) => g.resolved_document_id).filter(Boolean) as string[]));
    const faqIds  = Array.from(new Set(gaps.map((g) => g.resolved_faq_id).filter(Boolean) as string[]));

    const [deptsRes, usersRes, docsRes, faqsRes] = await Promise.all([
      deptIds.length ? context.supabase.from("departments").select("id, name").in("id", deptIds) : Promise.resolve({ data: [] as { id: string; name: string }[] }),
      userIds.length ? context.supabase.from("profiles").select("id, full_name").in("id", userIds) : Promise.resolve({ data: [] as { id: string; full_name: string | null }[] }),
      docIds.length  ? context.supabase.from("knowledge_documents").select("id, title, doc_code").in("id", docIds) : Promise.resolve({ data: [] as { id: string; title: string; doc_code: string | null }[] }),
      faqIds.length  ? context.supabase.from("faqs").select("id, question_en").in("id", faqIds) : Promise.resolve({ data: [] as { id: string; question_en: string | null }[] }),
    ]);
    ...
    return {
      gaps: gaps.map((g) => ({
        ...
        resolved_document: g.resolved_document_id ? docMap.get(g.resolved_document_id as string) ?? null : null,
...
export const updateKnowledgeGap = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase
      .from("knowledge_gaps").update(update as never).eq("id", id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const getKnowledgeGapStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data: gaps } = await context.supabase
      .from("knowledge_gaps")
      .select("id, status, occurrences, question_sample, department_id, confidence, resolution_date, first_seen, created_at, updated_at");
    const list = (gaps ?? []) as Array<{
      id: string; status: string; occurrences: number;
      question_sample: string; department_id: string | null;
      confidence: number | null; resolution_date: string | null;
      first_seen: string; updated_at: string;
    }>;
    const open = list.filter((g) => g.status === "open" || g.status === "in_progress");
    const monthAgo = new Date(); monthAgo.setDate(monthAgo.getDate() - 30);
    const resolvedMonth = list.filter((g) =>
    ...
    const deptIds = Array.from(new Set(open.map((g) => g.department_id).filter(Boolean) as string[]));
    ...
    const avgResolutionHours = resolved.length
      ? resolved.reduce((sum, g) => sum + (new Date(g.resolution_date!).getTime() - new Date(g.first_seen).getTime()), 0)
