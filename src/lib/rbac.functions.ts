...
export const listAssignableRoles=createServerFn({method:"GET"}).middleware([requireAuth]).handler(async({context})=>{await requireAnyPermission(context,["user.create","user.update","platform.manage"]);return getRbacAdminRepository(context.supabase).listRoles();});
