...
export const listAssignableRoles=createServerFn({method:"GET"}).middleware([requireAuth]).handler(async({context})=>{await requireAnyPermission(context,["user.create","user.update","platform.manage"]);const roles=await getRbacAdminRepository(context.supabase).listRoles();return roles.filter((r)=>r.key!=="platform_owner"&&r.key!=="platform_admin");});
