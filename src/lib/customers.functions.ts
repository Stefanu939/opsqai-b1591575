  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.format === "md") {
      ...
      const body = blocks.map((b) => {
        ...
        if (b.type === "callout") return `<aside class="callout callout-${b.kind ?? "note"}"><strong>${esc((b.kind ?? "note").toUpperCase())}</strong>${b.title ? `<h4>${esc(b.title)}</h4>` : ""}<p>${esc(b.text)}</p></aside>`;
