...
export const getKnowledgeAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: questions }, { data: msgs }, { data: gaps }, { data: docs }, { data: faqs }, { data: fb }] = await Promise.all([
      supa.from("audit_log").select("question, sources, created_at").gte("created_at", sinceIso).limit(2000),
      supa.from("messages").select("role, confidence, created_at").eq("role", "assistant").gte("created_at", sinceIso).limit(5000),
      supa.from("knowledge_gaps").select("id, status, occurrences, question_sample").eq("status", "open"),
    ...
    const topQuestions = Object.entries(qCounts).sort((a, b) => b[1] - a[1]).slice(0, 10);

    // Most used SOPs
    const docCounts: Record<string, { title: string; count: number }> = {};
    for (const r of questions ?? []) {
      const srcs = (r.sources ?? []) as Array<{ type: string; document_id?: string; title?: string }>;
      for (const s of srcs) {
        if (s.type === "document" && s.document_id) {
          docCounts[s.document_id] = { title: s.title ?? "Document", count: (docCounts[s.document_id]?.count ?? 0) + 1 };
        }
      }
    }
    const topDocs = Object.entries(docCounts).map(([id, v]) => ({ id, ...v })).sort((a, b) => b.count - a.count).slice(0, 10);
    ...
    const topFaqs = Object.entries(faqCounts).map(([id, v]) => ({ id, ...v })).sort((a, b) => b.count - a.count).slice(0, 10);

    const confidences = (msgs ?? []).map((m) => (m as { confidence: number | null }).confidence).filter((x): x is number => typeof x === "number");
    ...
    const usage = Object.entries(usageByDay).sort().map(([day, count]) => ({ day, count }));

    const positive = (fb ?? []).filter((f) => f.rating === 1).length;
    const negative = (fb ?? []).filter((f) => f.rating === -1).length;

    return {
      topQuestions, topDocs, topFaqs,
      openGaps: gaps ?? [],
      unansweredCount: (gaps ?? []).reduce((a, g) => a + g.occurrences, 0),
      lowConfidenceCount,
      outdatedSops: outdatedSops.map((d) => ({ id: d.id, title: d.title, doc_code: d.doc_code, updated_at: d.updated_at })),
      avgConfidence,
