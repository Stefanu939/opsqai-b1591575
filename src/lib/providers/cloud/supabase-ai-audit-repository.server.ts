...
export function createSupabaseAiAuditRepository(client:SupabaseClient<Database>):IAiAuditRepository{return{
  async list(companyId,limit){const{data,error}=await client.from("ai_audits").select("id,score,maturity,passed,warnings,critical,summary,created_at").eq("company_id",companyId).order("created_at",{ascending:false}).limit(limit);if(error)throw new Error(error.message);return(data??[]).map((r)=>({id:r.id,score:r.score,maturity:r.maturity,passed:r.passed,warnings:r.warnings,critical:r.critical,summary:JSON.parse(JSON.stringify(r.summary)) as JsonLike,createdAt:r.created_at}));},
