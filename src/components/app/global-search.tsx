...
export function GlobalSearch({ asButton = false }: { asButton?: boolean }) {
  ...
  useEffect(() => {
    ...
    const h = setTimeout(async () => {
      try { const r = await search({ data: { q, companyId: activeCompanyId ?? null } }); setResults(r.results as any[]); }
      catch { setResults([]); }
    }, 180);
  ...
  return (
    <>
      {asButton && (
        <Button variant="ghost" size="sm" onClick={() => setOpen(true)} className="h-8 gap-2 text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent">
          <Search className="h-3.5 w-3.5" />
          <span className="text-xs hidden lg:inline">Search</span>
          <kbd className="hidden lg:inline-flex pointer-events-none h-4 select-none items-center gap-1 rounded border bg-muted px-1 font-mono text-[10px] font-medium text-muted-foreground">⌘K</kbd>
        </Button>
      )}
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput value={q} onValueChange={setQ} placeholder="Search SOPs, FAQs, users, audit…" />
        <CommandList>
            ...
            return (
              <div key={kind}>
                ...
                <CommandGroup heading={LABELS[kind] ?? kind}>
                  {items.map((r) => (
                    <CommandItem key={`${r.kind}-${r.id}`} value={`${r.kind}-${r.label}-${r.id}`} onSelect={() => go(r)}>
