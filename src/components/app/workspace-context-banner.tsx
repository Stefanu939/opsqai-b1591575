...
export function WorkspaceContextBanner() {
  ...
  return (
    <div className="border-b border-border bg-muted/30 px-4 md:px-8 py-2 flex items-center gap-2 text-[11px]">
      ...
      {isGlobal ? (
        <Badge variant="secondary" className="gap-1.5"><Globe2 className="h-3 w-3" /> All Companies (Global)</Badge>
      ) : (
        <Badge variant="default" className="gap-1.5"><Building2 className="h-3 w-3" /> {companyName ?? "—"}</Badge>
      )}
      <span className="ml-auto text-muted-foreground hidden sm:inline">Platform context active</span>
    </div>
  );
}
