...
import { GraduationCap, BookOpen, Award, Library, BarChart3, Settings as SettingsIcon } from "lucide-react";
...
export function AcademySubnav() {
  ...
  return (
    <nav className="flex flex-wrap items-center gap-1 border-b border-border/60 px-4 md:px-6 py-2 bg-background/80 backdrop-blur sticky top-0 z-20">
      ...
      <Link to="/app/academy/kb" className={baseCls}>
        <Library className="h-3.5 w-3.5" /> Academy Knowledge Base
      </Link>
      {canManage && (
        <>
