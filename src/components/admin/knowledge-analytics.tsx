...
export function KnowledgeAnalyticsPage() {
  ...
  if (!data)
    return <div className="flex-1 p-8 text-sm text-muted-foreground">Loading analytics…</div>;
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `opsqai-analytics-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Section title="Top questions">
          {data.topQuestions.length === 0 ? (
            ...
            data.topQuestions.map(([q, c]) => <Row key={q} label={q} value={c} />)
          )}
        </Section>
        <Section title="Most used SOPs">
          {data.topDocs.length === 0 ? (
            ...
            data.topDocs.map((d) => <Row key={d.id} label={d.title} value={d.count} />)
          )}
        </Section>
        <Section title="Most used FAQs">
          {data.topFaqs.length === 0 ? (
            ...
            data.topFaqs.map((d) => <Row key={d.id} label={d.title} value={d.count} />)
          )}
        </Section>
        <Section title="Outdated SOPs (>6 months)">
          {data.outdatedSops.length === 0 ? (
            <Empty />
          ) : (
            data.outdatedSops.map((d) => (
              <Row
                key={d.id}
                label={`${d.doc_code ? d.doc_code + " · " : ""}${d.title}`}
...
function Kpi({ label, value, tone }: { label: string; value: number | string; tone?: "warn" }) {
  return (
    <Card className="p-4">
      <div
        className={`text-2xl font-bold ${tone === "warn" && Number(value) > 0 ? "text-amber-600" : ""}`}
      >
        {value}
      </div>
