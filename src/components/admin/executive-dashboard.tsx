...
export function ExecutiveDashboard() {
  ...
  useEffect(() => {
    ...
    activityFn({
      ...
      .then((r) => setActivity((r.rows as any[]) ?? []))
      .catch(() => setActivity([]));
  ...
  if (!hasPermission("dashboard.view")) {
    return (
      <div className="p-8 text-sm text-muted-foreground">
        You don't have access to the Executive Dashboard.
      </div>
    );
  }
  ...
  const kpiCards = [
    {
      ...
      value: k.questionsAnswered ?? 0,
      icon: MessageSquare,
      sub: `${k.questionsToday ?? 0} today`,
    },
    {
      label: "AI Confidence",
      value: `${Math.round((Number(k.avgConfidence) || 0) * 100)}%`,
      icon: Brain,
      sub: "Last 30d",
    },
      ...
      <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Live KPIs, health and AI insights for your workspace.
          </p>
        </div>
          ...
          {lastAudit ? (
            <>
              <div className="text-3xl font-semibold tabular-nums">
                {Math.round(lastAudit.score)}/100
              </div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                <div>
                  <div className="text-lg font-semibold text-emerald-500">{lastAudit.passed}</div>
                  <div className="text-[10px] uppercase text-muted-foreground">Passed</div>
                </div>
                <div>
                  <div className="text-lg font-semibold text-amber-500">{lastAudit.warnings}</div>
                  <div className="text-[10px] uppercase text-muted-foreground">Warnings</div>
                </div>
                <div>
                  <div className="text-lg font-semibold text-red-500">{lastAudit.critical}</div>
