...
function AppFrame({
  ...
  title: string;
  sidebar: { icon: ElementType; label: string; active?: boolean }[];
  children: ReactNode;
}) {
  return (
...
function SceneCompany(t: number) {
  ...
  return (
    <AppFrame
      ...
      sidebar={NAV_PLATFORM.map((s) => ({ ...s, active: s.label === "Companies" }))}
    >
              ...
              .map((c: any) => (
                <div
                  ...
                  className={cn(
                    ...
                    c.fresh &&
                      "ring-2 ring-primary/40 bg-primary/5 animate-in fade-in slide-in-from-bottom-2",
                  )}
                >
...
function SceneKnowledge(t: number) {
  ...
  return (
    <AppFrame
      ...
      sidebar={NAV_WORKSPACE.map((s) => ({ ...s, active: s.label === "Knowledge" }))}
    >
        ...
        <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
          ...
          {docs.map((d, i) => {
            ...
            return (
              <div
                key={d.code}
                className="grid grid-cols-[24px_120px_1fr_100px] items-center gap-3 px-4 py-2.5 border-b border-border/60 last:border-0 text-[12px]"
              >
                ...
                <span
                  className={cn(
                    "justify-self-end px-2 py-0.5 rounded-md text-[11px] font-medium",
                    color,
                  )}
                >
                  {status}
...
function SceneFAQ(t: number) {
  const faqs = [
    {
      q: "What is the maximum receiving time for a truck?",
      a: "Receiving must be completed within 90 minutes of dock assignment.",
    },
    {
      q: "Where do damaged goods go?",
      a: "To the QC quarantine area — never to standard putaway.",
    },
    { q: "When is hearing protection mandatory?", a: "In every zone marked above 80 dB." },
    {
      q: "Who signs off on outbound loads?",
      a: "The loader signs the loading sheet; the driver signs the CMR.",
    },
  ];
  return (
...
function SceneAssistant(t: number, lang: Lang) {
  ...
  return (
    <AppFrame
      title="AI Assistant"
      sidebar={NAV_WORKSPACE.map((s) => ({ ...s, active: s.label === "AI Assistant" }))}
    >
      <div className="grid grid-cols-[1fr_260px] gap-4 h-full">
        ...
        <div className="rounded-xl border border-border/60 bg-card p-4">
          ...
          <div className="mt-3 text-[11px] text-muted-foreground leading-relaxed border-l-2 border-primary/40 pl-2">
            "Receiving must be completed within 90 minutes of dock assignment. If receiving exceeds
            60 minutes, the team leader must be informed."
          </div>
...
const MULTI_LANGS: { code: string; label: string; q: string; a: string; src: string }[] = [
  {
    code: "en",
    label: "English",
    q: "What is the maximum receiving time?",
    a: "Receiving must be completed within 90 minutes of dock assignment.",
    src: "Sources:",
  },
  {
    code: "de",
    label: "Deutsch",
    q: "Was ist die maximale Empfangszeit?",
    a: "Der Empfang muss innerhalb von 90 Minuten nach Zuweisung der Rampe abgeschlossen sein.",
    src: "Quellen:",
  },
  {
    code: "ro",
    label: "Română",
    q: "Care este timpul maxim de recepție?",
    a: "Recepția trebuie finalizată în 90 de minute de la alocarea rampei.",
    src: "Surse:",
  },
  {
    code: "fr",
    label: "Français",
    q: "Quel est le temps de réception maximum ?",
    a: "La réception doit être terminée dans les 90 minutes suivant l'affectation du quai.",
    src: "Sources :",
  },
  {
    code: "es",
    label: "Español",
    q: "¿Cuál es el tiempo máximo de recepción?",
    a: "La recepción debe completarse en 90 minutos tras la asignación del muelle.",
    src: "Fuentes:",
  },
  {
    code: "pl",
    label: "Polski",
    q: "Jaki jest maksymalny czas przyjęcia?",
    a: "Przyjęcie musi zakończyć się w ciągu 90 minut od przydzielenia rampy.",
    src: "Źródła:",
  },
  {
    code: "it",
    label: "Italiano",
    q: "Qual è il tempo massimo di ricezione?",
    a: "La ricezione deve concludersi entro 90 minuti dall'assegnazione della banchina.",
    src: "Fonti:",
  },
  {
    code: "sv",
    label: "Svenska",
    q: "Vad är den maximala mottagningstiden?",
    a: "Mottagningen måste slutföras inom 90 minuter efter kajtilldelning.",
    src: "Källor:",
  },
  {
    code: "ja",
    label: "日本語",
    q: "受入時間の上限は？",
    a: "受け入れはドック割り当てから 90 分以内に完了する必要があります。",
    src: "出典:",
  },
  {
    code: "ko",
    label: "한국어",
    q: "최대 입고 시간은?",
    a: "입고는 도크 배정 후 90분 이내에 완료해야 합니다.",
    src: "출처:",
  },
  {
    code: "zh",
    label: "中文",
    q: "最长收货时间是多少？",
    a: "收货必须在指派月台后 90 分钟内完成。",
    src: "来源:",
  },
  {
    code: "ar",
    label: "العربية",
    q: "ما الحد الأقصى لوقت الاستلام؟",
    a: "يجب إتمام الاستلام خلال 90 دقيقة من تخصيص الرصيف.",
    src: "المصادر:",
  },
];
      ...
      <div className="grid grid-cols-[1fr_260px] gap-4 h-full" dir={rtl ? "rtl" : "ltr"}>
        ...
        <div className="rounded-xl border border-border/60 bg-card p-4 flex flex-col gap-2">
          ...
          <div className="mt-3 text-[11px] text-muted-foreground leading-relaxed">
            One knowledge base. Same citations. The interface, sources and answers adapt to every
            language your teams speak.
          </div>
...
function SceneAcademy(t: number) {
  ...
  return (
    <AppFrame
      title="Academy · Warehouse Onboarding"
      sidebar={NAV_WORKSPACE.map((s) => ({ ...s, active: s.label === "Academy" }))}
    >
      <div className="grid grid-cols-[1fr_260px] gap-4 h-full">
          ...
          {["Brakes tested", "Horn works", "Forks aligned"].map((q, i) => (
            ...
            >
              <CheckCircle2
                className={cn(
                  "h-3.5 w-3.5",
                  t > 0.3 + i * 0.15 ? "text-emerald-600" : "text-muted-foreground",
                )}
              />
              {q}
            </div>
...
function SceneEnterpriseDocs(t: number) {
  ...
  return (
    <AppFrame
      title="Enterprise Documents · SOP Generator"
      sidebar={NAV_WORKSPACE.map((s) => ({ ...s, active: s.label === "Governance" }))}
    >
      <div className="grid grid-cols-[240px_1fr] gap-4 h-full">
        <div className="rounded-xl border border-border/60 bg-card p-4 flex flex-col gap-2">
          ...
          <div className="rounded-md border border-primary/40 bg-primary/5 px-3 py-2 text-[12px] font-medium">
            Standard Operating Procedure
          </div>
          <div className="rounded-md border border-border/60 px-3 py-2 text-[12px] text-muted-foreground">
            Compliance Report
          </div>
          <div className="rounded-md border border-border/60 px-3 py-2 text-[12px] text-muted-foreground">
        ...
        {[
          {
            icon: ShieldCheck,
            title: "Roles",
            body: "Admin · Manager · Team Leader · Employee",
            metric: "4 roles",
          },
          {
            icon: Search,
            title: "Knowledge gaps",
            body: "Unanswered questions tracked and assigned",
            metric: "3 open",
          },
          {
            icon: History,
            title: "Versioning",
            body: "Every SOP has audit-trailed versions",
            metric: "v3 latest",
          },
          {
            icon: MessageSquare,
            title: "Notifications",
            body: "Ack requests, gap resolutions, approvals",
            metric: "12 today",
          },
        ].map((c, i) => (
          <div
...
function SceneAudit(t: number) {
  ...
  return (
    <AppFrame
      title="Audit Log"
      sidebar={NAV_WORKSPACE.map((s) => ({ ...s, active: s.label === "Audit" }))}
    >
      <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
        {events.map((e, i) => (
          <div
...
const SCENES: SceneDef[] = [
  {
    ...
    narrationEn:
      "Every OPSQAI customer starts with their own isolated workspace — secure, branded, and completely separated from every other tenant.",
    narrationDe:
      "Jeder OPSQAI-Kunde beginnt mit seinem eigenen isolierten Workspace — sicher, individuell gebrandet und vollständig von anderen Mandanten getrennt.",
    render: SceneCompany,
  },
  {
    id: "users",
    ...
    narrationEn:
      "Enterprise role-based access — Admin, Manager, Team Leader, Employee — keeps every department aligned with what they need to see.",
    narrationDe:
      "Rollenbasierte Enterprise-Berechtigungen — Admin, Manager, Teamleiter, Mitarbeiter — halten jede Abteilung auf das ausgerichtet, was sie sehen muss.",
    render: SceneUsers,
  },
  {
    id: "kb",
    ...
    narrationEn:
      "SOPs, manuals and quality documents are uploaded, indexed and made semantically searchable — grounded in your company's real procedures.",
    narrationDe:
      "SOPs, Handbücher und Qualitätsdokumente werden hochgeladen, indexiert und semantisch durchsuchbar gemacht — verankert in den echten Prozessen Ihres Unternehmens.",
  ...
  },
  {
    ...
    narrationEn:
      "Frequent operational questions are captured as curated FAQs, so teams never search twice for the same answer.",
    narrationDe:
      "Häufige operative Fragen werden als kuratierte FAQs erfasst, damit Teams nie zweimal nach derselben Antwort suchen müssen.",
    render: SceneFAQ,
  },
  {
    id: "assistant",
    ...
    narrationEn:
      "Ask a question. OPSQAI answers only from your knowledge base — every response is source-backed, with the exact document, section and confidence.",
    narrationDe:
      "Stellen Sie eine Frage. OPSQAI antwortet ausschließlich aus Ihrer Wissensdatenbank — mit Quellenangabe, exaktem Dokument, Abschnitt und Konfidenz.",
  ...
  },
  {
    ...
    narrationEn:
      "The same question. The same sources. The same answer — delivered in virtually every language your workforce speaks. Nothing else changes.",
    narrationDe:
      "Dieselbe Frage. Dieselben Quellen. Dieselbe Antwort — in praktisch jeder Sprache, die Ihre Belegschaft spricht. Nichts anderes ändert sich.",
  ...
  },
  {
    ...
    narrationEn:
      "Learning paths, lessons, quizzes and certificates — turn your SOPs into structured onboarding without leaving the platform.",
    narrationDe:
      "Lernpfade, Lektionen, Quizze und Zertifikate — machen Sie aus Ihren SOPs strukturiertes Onboarding, ohne die Plattform zu verlassen.",
    render: SceneAcademy,
  },
  {
    id: "docs",
    ...
    narrationEn:
      "Generate SOPs, compliance reports and training material — drafted from your existing knowledge, ready to review and publish.",
    narrationDe:
      "Generieren Sie SOPs, Compliance-Berichte und Schulungsunterlagen — aus Ihrem vorhandenen Wissen entworfen, bereit zur Freigabe.",
  ...
  },
  {
    ...
    narrationEn:
      "Roles, versioning, knowledge gaps and notifications — the governance layer enterprises expect, built in from day one.",
    narrationDe:
      "Rollen, Versionierung, Wissenslücken und Benachrichtigungen — die Governance-Ebene, die Enterprises erwarten, von Anfang an integriert.",
  ...
  },
  {
    ...
    narrationEn:
      "Every question, every document change, every role update — recorded in a per-tenant audit log your compliance team can rely on.",
    narrationDe:
      "Jede Frage, jede Dokumentänderung, jede Rollenaktualisierung — protokolliert in einem mandantenspezifischen Audit-Log, auf das sich Ihr Compliance-Team verlassen kann.",
    render: SceneAudit,
  },
  {
    id: "final",
    ...
    narrationEn:
      "OPSQAI. Enterprise AI platform for operational knowledge. Transform company knowledge into operational excellence.",
    narrationDe:
      "OPSQAI. Enterprise-KI-Plattform für operatives Wissen. Verwandeln Sie Unternehmenswissen in operative Exzellenz.",
    render: SceneFinal,
  },
];
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-16 md:py-24">
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          <h2 className="mt-3 text-3xl md:text-4xl font-semibold tracking-tight">
            See OPSQAI in action
          </h2>
          <p className="mt-4 text-[15px] text-muted-foreground leading-relaxed">
            Follow a complete operational workflow — from company creation to enterprise governance
        ...
        >
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-16 flex justify-center px-6">
            <div
              key={scene.id + ":" + lang}
              className="max-w-2xl text-center animate-in fade-in slide-in-from-bottom-2 duration-500"
            >
            ...
            {SCENES.map((s, i) => {
              ...
              return (
                <div
                  key={s.id}
                  style={{ width: `${w}%` }}
                  className={cn("h-2", i > 0 && "border-l border-background/60")}
                />
              );
            })}
          </div>
