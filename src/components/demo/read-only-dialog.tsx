...
export function useDemoBlockHandler() {
  ...
  return (feature?: string) => (e?: { preventDefault?: () => void; stopPropagation?: () => void }) => {
    e?.preventDefault?.();
    e?.stopPropagation?.();
    show(feature);
  };
...
export function DemoReadOnlyProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <DemoReadOnlyContext.Provider value={{ show: (f) => { setFeature(f); setOpen(true); } }}>
      {children}
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="pt-1 leading-relaxed">
              The Interactive Demo is intentionally read-only so you can explore the platform safely.
              {feature ? <> <span className="text-foreground/80">— <em>{feature}</em> is one of the actions reserved for your tenant.</span></> : null}
              {" "}During implementation, your organization will have full administrative capabilities tailored to your roles and permissions.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-2">
            <Button variant="outline" onClick={() => setOpen(false)}>Keep exploring</Button>
