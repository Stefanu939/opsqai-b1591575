...
export function AssignTrainingDialog({
  open, onOpenChange, defaultPathId, onAssigned,
}: {
  open: boolean;
  ...
  const submit = async () => {
    ...
    try {
      ...
      onAssigned?.();
      // reset
      setUserIds([]); setDeptIds([]); setRoles([]); setEntireCompany(false);
    } catch (e: any) {
        ...
        {!targets ? (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-xs uppercase tracking-wide text-muted-foreground">Courses</Label>
              <ScrollArea className="h-32 rounded border p-2">
                {targets.paths.length === 0 ? (
                  <div className="text-xs text-muted-foreground italic p-2">No published courses yet.</div>
                ) : targets.paths.map((p) => (
                  <label key={p.id} className="flex items-center gap-2 py-1 px-1 rounded hover:bg-accent/60 text-sm cursor-pointer">
                    <Checkbox checked={pathIds.includes(p.id)} onCheckedChange={() => toggle(pathIds, p.id, setPathIds)} />
            ...
            <Tabs defaultValue="users">
              ...
              <TabsContent value="users" className="space-y-2">
                <div className="relative">
                  ...
                  <Input placeholder="Search users" value={userQuery} onChange={(e) => setUserQuery(e.target.value)} className="pl-7 h-9" />
                </div>
                <ScrollArea className="h-40 rounded border p-2">
                  {filteredUsers.map((u) => (
                    <label key={u.id} className="flex items-center gap-2 py-1 px-1 rounded hover:bg-accent/60 text-sm cursor-pointer">
                      <Checkbox checked={userIds.includes(u.id)} onCheckedChange={() => toggle(userIds, u.id, setUserIds)} />
                      <span className="flex-1 truncate">{u.name}</span>
                    </label>
                  ))}
                </ScrollArea>
                {userIds.length > 0 && <div className="text-xs text-muted-foreground">{userIds.length} user{userIds.length === 1 ? "" : "s"} selected</div>}
              </TabsContent>
              <TabsContent value="departments">
                <ScrollArea className="h-40 rounded border p-2">
                  {targets.departments.map((d) => (
                    <label key={d.id} className="flex items-center gap-2 py-1 px-1 rounded hover:bg-accent/60 text-sm cursor-pointer">
                      <Checkbox checked={deptIds.includes(d.id)} onCheckedChange={() => toggle(deptIds, d.id, setDeptIds)} />
                      <span className="flex-1">{d.name}</span>
                    </label>
                  ))}
                  {targets.departments.length === 0 && <div className="text-xs text-muted-foreground italic p-2">No departments yet.</div>}
                </ScrollArea>
              </TabsContent>
              <TabsContent value="roles">
                <ScrollArea className="h-40 rounded border p-2">
                  {targets.roles.map((r) => (
                    <label key={r} className="flex items-center gap-2 py-1 px-1 rounded hover:bg-accent/60 text-sm cursor-pointer">
                      <Checkbox checked={roles.includes(r)} onCheckedChange={() => toggle(roles, r, setRoles)} />
              ...
              <TabsContent value="company" className="pt-2">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <Checkbox checked={entireCompany} onCheckedChange={(v) => setEntireCompany(Boolean(v))} />
                  Assign to <strong>every active member</strong> of the company ({targets.users.length})
                </label>
              </TabsContent>
            </Tabs>
            ...
            <div className="flex flex-wrap gap-4 pt-1">
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={mandatory} onCheckedChange={(v) => setMandatory(Boolean(v))} /> Mark as mandatory
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={notify} onCheckedChange={(v) => setNotify(Boolean(v))} /> Notify learners
              </label>
            </div>

            <div className="text-xs text-muted-foreground rounded bg-muted/40 p-2">
              Estimated <strong>~{totalUsers}</strong> learner{totalUsers === 1 ? "" : "s"} · {pathIds.length} course{pathIds.length === 1 ? "" : "s"}
            </div>
          </div>
        )}
