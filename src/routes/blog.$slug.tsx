...
export const Route = createFileRoute("/blog/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [{ title: "Article not found — OPSQAI" }, { name: "robots", content: "noindex" }],
      };
    }
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-24 text-center">
      ...
      <p className="mt-3 text-muted-foreground">
        This piece may have moved or is not yet published.
      </p>
      <Link
        to="/blog"
    ...
    <main className="mx-auto max-w-3xl px-6 py-20">
      ...
      <header className="mb-10">
        ...
        <p className="mt-6 text-sm text-muted-foreground">
          By <span className="text-foreground">{post.author.name}</span>, {post.author.role} ·{" "}
        ...
        <footer className="mt-16 rounded-2xl border border-border/60 bg-card/50 p-6">
          ...
          <ul className="mt-4 space-y-2 text-sm">
            {post.relatedLandingPages?.map((p) => (
              <li key={p}>
                <a href={p} className="text-foreground hover:text-primary">
                  {p}
                </a>
              </li>
            ))}
            {post.relatedPosts?.map((s) => (
              <li key={s}>
                <a href={`/blog/${s}`} className="text-foreground hover:text-primary">
                  /blog/{s}
                </a>
              </li>
            ))}
