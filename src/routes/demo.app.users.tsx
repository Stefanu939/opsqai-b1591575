  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("profiles")
        .select("id,full_name,first_name,last_name,position,language_pref,is_active,created_at")
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Roles are stored separately from profiles — a security best practice enforced by RLS.
          </p>
        </div>
            ...
            <Card key={p.id} className="p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground truncate">
                  {persona?.position ?? p.position} · {persona?.department}
                </div>
              </div>
              <span className={`chip ${meta.tone}`}>{meta.label}</span>
              <Button variant="ghost" size="sm" onClick={() => show("Change role or permissions")}>
                Manage
              </Button>
            </Card>
          );
