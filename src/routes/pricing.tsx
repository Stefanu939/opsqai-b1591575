...
const PRICING_FAQS = [
  {
    question: "How is OPSQAI priced?",
    answer:
      "Per user per month, with a volume-tiered rate that decreases as the deployment scales. A one-time implementation package covers onboarding, initial knowledge ingestion and workspace configuration.",
  },
  {
    question: "Is there a free trial?",
    answer:
      "OPSQAI offers a live, read-only interactive demo populated with real logistics operations data. Paid pilots are available on request for qualified enterprise buyers.",
  },
  {
    question: "Where is our data hosted?",
    answer:
      "OPSQAI hosts customer data in the EU. Data residency, retention and processing terms are covered in the DPA included with every enterprise contract.",
  },
  {
    question: "Do you support SSO?",
    answer:
      "Yes — SSO is available on request for enterprise plans, alongside role-based access control across departments and sites.",
  },
];
    ...
    <MarketingLayout>
      <section className="mx-auto max-w-4xl px-4 py-16 md:py-24 text-center">
        ...
        <p className="mt-5 text-lg text-muted-foreground">
          From <span className="font-semibold text-foreground">€8 / user / month</span>, plus a
          one-time implementation package sized to your rollout. Every feature is included from day
          one.
        </p>
      </section>
        ...
        <Card className="p-6 md:p-8 border-primary/30 shadow-xl">
          ...
          <div className="mt-4 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            ...
            <div className="text-right">
              ...
              <div className="text-2xl font-semibold text-primary">
                {eur.format(rate)}
                <span className="text-sm text-muted-foreground font-normal"> / user / mo</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            <Stat
              ...
              value={eur.format(monthly)}
              sub={`${eur.format(rate)} × ${users.toLocaleString()} users`}
            />
            <Stat
              icon={Building2}
              label="Annual subscription"
              value={eur.format(annual)}
              sub="Billed annually"
            />
            <Stat
              icon={ShieldCheck}
              label="Implementation (one-time)"
              value={`${eur.format(implLo)} – ${eur.format(implHi)}`}
              sub="Setup, migration, training"
            />
          </div>

          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            <Button asChild className="flex-1">
              <Link to="/contact">Get a written quote</Link>
            </Button>
          ...
          <p className="mt-4 text-[11px] text-muted-foreground">
            Estimates only. Final pricing depends on rollout scope, integrations, languages and
            support level. Volume discounts apply automatically as your team grows.
          </p>
        </Card>
      </section>
          ...
          <div className="mt-2 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h2 className="text-2xl font-semibold tracking-tight">
                Everything, for every operations team
              </h2>
      ...
      <section className="mx-auto max-w-3xl px-4 pb-20 text-center text-sm text-muted-foreground">
        <p>
          Every deployment includes EU hosting, multi-tenant isolation, audit log, role-based
          access, OPSQAI Academy and source-grounded answering. Onboarding and success management
          are part of the implementation package — never a paid add-on.
        </p>
      </section>
    </MarketingLayout>
  );
