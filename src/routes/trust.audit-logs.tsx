  ...
  head: () => ({
    meta: [
      { title: "Audit Logs — OPSQAI Trust Center" },
      {
        name: "description",
        content:
          "OPSQAI records questions, sources, document changes, role changes and admin actions in a per-tenant audit log.",
      },
  ...
  component: () => (
    ...
    >
      ...
      <ul>
        ...
        <li>
          The raw response from the AI model — we store the sources and the visible answer, not
      ...
      <ul>
        ...
        <li>
          Audit rows are scoped by <code>company_id</code> and locked down with row-level security.
        </li>
        <li>
          Platform super-admins can review audit logs across tenants for incident response only;
          this access is itself logged.
        </li>
      </ul>
      <h2>Retention</h2>
      <p>
        Audit logs are retained for the lifetime of the tenant subscription. When a tenant is
        terminated, audit entries are anonymized on purge — module, action, resource, severity,
        success and event timestamp are kept under a hashed tenant label in a separate archive; user
        IDs and payloads are not archived. The anonymized archive is retained for a{" "}
        <strong>rolling 24 months</strong>, then purged. See{" "}
        <a href="/trust/data-retention">Data retention</a> for the full 30-day termination flow.
      </p>
    </TrustTopic>
  ),
});
