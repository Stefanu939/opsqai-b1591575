...
const SOCIAL_ASSETS: Asset[] = [
  { label: "LinkedIn — profile (400×400)", href: "/brand/social/linkedin-profile-400.png", ext: "PNG" },
  { label: "LinkedIn — banner (1584×396)", href: "/brand/social/linkedin-banner-1584x396.png", ext: "PNG" },
  { label: "LinkedIn — cover (1128×191)", href: "/brand/social/linkedin-cover-1128x191.png", ext: "PNG" },
  { label: "Instagram — profile (1080)", href: "/brand/social/instagram-profile-1080.png", ext: "PNG" },
  { label: "Instagram — post (1080×1080)", href: "/brand/social/instagram-post-1080x1080.png", ext: "PNG" },
  { label: "Instagram — story (1080×1920)", href: "/brand/social/instagram-story-1080x1920.png", ext: "PNG" },
  { label: "X / Twitter — banner (1500×500)", href: "/brand/social/x-banner-1500x500.png", ext: "PNG" },
  { label: "Facebook — cover (1640×624)", href: "/brand/social/facebook-cover-1640x624.png", ext: "PNG" },
  { label: "YouTube — banner (2560×1440)", href: "/brand/social/youtube-banner-2560x1440.png", ext: "PNG" },
  { label: "Product Hunt — thumbnail (1000)", href: "/brand/social/product-hunt-1000.png", ext: "PNG" },
  ...
  return (
    ...
    >
      <div className="min-w-0">
        ...
        <div className="text-[11px] uppercase tracking-wider text-muted-foreground">{asset.ext}</div>
      </div>
...
function Swatch({ name, hex, token }: { name: string; hex: string; token: string }) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border bg-card p-3">
      <div className="h-12 w-12 rounded-md shrink-0 border border-border" style={{ background: hex }} />
      <div className="min-w-0">
        <div className="text-sm font-medium truncate">{name}</div>
        <div className="font-mono text-[11px] text-muted-foreground">{hex}</div>
        <div className="text-[10px] uppercase tracking-wider text-muted-foreground/70 truncate">{token}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 md:px-6 py-10 space-y-12">
      <header className="space-y-3">
        ...
        <h1 className="text-3xl md:text-4xl font-display font-semibold tracking-tight">OPSQAI Visual Identity</h1>
        <p className="max-w-2xl text-muted-foreground">
          The single source of truth for every OPSQAI surface — logo, color, typography, and ready-to-ship
          assets for product, web, marketing and the app stores. Download SVG and PNG below; everything you
          see in the app loads from these same files.
        </p>
      </header>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="p-8 flex flex-col items-center justify-center gap-4 bg-[#020617] min-h-[180px]">
            <Logo size={36} />
            <span className="text-[11px] uppercase tracking-wider text-white/50">Horizontal · dark</span>
          </Card>
          <Card className="p-8 flex flex-col items-center justify-center gap-4 bg-white text-slate-900 min-h-[180px]">
            <Logo size={36} />
            <span className="text-[11px] uppercase tracking-wider text-slate-500">Horizontal · light</span>
          </Card>
          <Card className="p-8 flex flex-col items-center justify-center gap-4 min-h-[180px]">
            <LogoStacked size={72} />
            <span className="text-[11px] uppercase tracking-wider text-muted-foreground">Stacked</span>
          </Card>
      ...
      <section className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground max-w-2xl">
          Deep navy provides authority. Enterprise blue signals AI interaction. Teal marks operational
          success. The palette is WCAG AA across both themes.
        </p>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {COLOR_TOKENS.map((c) => <Swatch key={c.hex} {...c} />)}
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="p-6">
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2">Display — Space Grotesk</div>
            <div className="font-display text-4xl font-semibold tracking-tight">Operational Knowledge.</div>
            <div className="font-display text-2xl font-medium text-muted-foreground mt-1">Built for warehouses.</div>
          </Card>
          <Card className="p-6">
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2">Body — Inter</div>
            <p className="text-base">
              OPSQAI grounds every answer in your SOPs, manuals and FAQs. No hallucinations, full audit
              trail, multilingual by default — the operational standard for enterprise AI.
            </p>
          </Card>
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {LOGO_ASSETS.map((a) => <DownloadCard key={a.href} asset={a} />)}
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {SOCIAL_ASSETS.map((a) => <DownloadCard key={a.href} asset={a} />)}
        </div>
      </section>
