...
function RequestsPage() {
  ...
  const [filter, setFilter] = useState<"open" | "in_review" | "answered" | "closed" | "all">(
    "open",
  );
          ...
          <div className="flex items-center gap-2">
            ...
            {counts.open > 0 && (
              <Badge variant="default">
                {counts.open} {T("open").toLowerCase()}
              </Badge>
            )}
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      <Badge variant={statusVariant(r.status)} className="capitalize">
                        {T(r.status === "in_review" ? "inReview" : r.status)}
                      </Badge>
                      {r.priority !== "normal" && (
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={busy || !qEn || !qDe || !aEn || !aDe || !cat}>
            {T("submit")}
          </Button>
        </DialogFooter>
      </DialogContent>
...
function KbDialog({
  ...
  const [content, setContent] = useState(
    req.answer ? `Q: ${req.question}\n\nA: ${req.answer}` : `Q: ${req.question}\n\nA: `,
  );
  const [busy, setBusy] = useState(false);
  const valid =
    title.trim().length >= 3 && category.trim() && owner.trim() && content.trim().length >= 20;
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Formal company knowledge requires title, category, and source owner.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <Input
              ...
              placeholder="e.g. Night Shift Overtime Approval"
            />
          </div>
          <div>
            <label className="text-xs font-medium">{T("category")} *</label>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="optional, e.g. SOP-042"
            />
          </div>
          <div className="col-span-2">
            ...
            <Input
              ...
              placeholder="e.g. Operations Manager — A. Müller"
            />
          </div>
          <div className="col-span-2">
