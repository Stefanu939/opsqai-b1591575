...
import { listAuditCompanies, listAuditUsers, listAuditEntries } from "@/lib/exports.functions";
  ...
  if (scopedCompany && !scopedUser)
    return (
      <UsersView
        ...
        back={
          isPlatformAdmin
            ? () => navigate({ to: "/app/admin/audit", search: {} as never })
            : undefined
        }
      />
    );
    ...
    return (
      <EntriesView
        ...
        back={() =>
          navigate({
            to: "/app/admin/audit",
            search: (isPlatformAdmin ? { company: scopedCompany } : {}) as never,
          })
        }
      />
    );
...
function CompaniesView() {
  ...
  useEffect(() => {
    list()
      .then((r) => setRows(r as unknown as CompanyRow[]))
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {rows.map((c) => (
            <button
              key={c.id}
              onClick={() =>
                navigate({ to: "/app/admin/audit", search: { company: c.id } as never })
              }
              className="text-left"
            >
...
function UsersView({ companyId, back }: { companyId: string; back?: () => void }) {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    list({ data: { company_id: companyId } })
      .then((r) => setRows(r as unknown as UserRow[]))
      .finally(() => setLoading(false));
  }, [companyId, list]);
...
function EntriesView({
  ...
  }, [companyId, userId, moduleFilter, action, severity, from]);

  const modules = useMemo(
    () => Array.from(new Set(rows.map((r) => r.module).filter(Boolean))),
    [rows],
  );
            ...
            <div key={r.id} className="p-4">
              <div className="flex items-baseline justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {r.resource && (
                    <span className="text-xs text-muted-foreground font-mono">{r.resource}</span>
                  )}
                </div>
              ...
              {r.answerPreview && (
                <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{r.answerPreview}</p>
              )}
              {(r.oldValue != null || r.newValue != null) && (
                <details className="mt-2 text-xs">
                  ...
                  <div className="grid sm:grid-cols-2 gap-2 mt-2">
                    {r.oldValue !== null && (
                      <pre className="bg-muted rounded p-2 overflow-auto text-[11px]">
