    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-8 w-full">
        <section className="space-y-3">
          <h2 className="text-lg font-semibold flex items-center gap-2"><BookOpen className="h-5 w-5" /> My learning journey</h2>
          {enrollments.length === 0 ? (
            <Card className="p-8 text-center text-sm text-muted-foreground">
              No active courses yet. Use the AI welcome below to discover learning paths tailored to your role.
            </Card>
          ) : (
                ...
                <Card key={e.id} className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">{e.academy_learning_paths?.academy_departments?.name ?? "—"}</div>
                    </div>
                    <Badge variant={e.status === "completed" ? "default" : "secondary"}>{e.status}</Badge>
                  </div>
                  {e.due_at && (
                    <div className="text-[11px] text-muted-foreground flex items-center gap-1">
                      <Clock className="h-3 w-3" />Due {new Date(e.due_at).toLocaleDateString()}
                    </div>
                  )}
        ...
        <Card className="p-6 space-y-4">
          <div className="flex items-center gap-2 text-sm font-medium"><Sparkles className="h-4 w-4 text-primary" /> Discover learning paths for your role</div>
