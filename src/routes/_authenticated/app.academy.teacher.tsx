...
function AcademyHome() {
  ...
  useEffect(() => {
    void (async () => {
      try { setEnrollments(((await myEnroll()) as any[]) ?? []); } catch { /* */ }
    })();
  }, []);

  const findNextLesson = async (mode: "next" | "previous"): Promise<{ pathId: string; lessonId: string; enrollmentId: string } | null> => {
    const active = enrollments
      .filter((e) => e.status !== "completed")
      .sort((a, b) => new Date(b.last_activity_at ?? b.started_at ?? b.created_at).getTime() - new Date(a.last_activity_at ?? a.started_at ?? a.created_at).getTime());
    const target = active[0] ?? enrollments[0];
    if (!target) return null;
    try {
      const data = (await getPath({ data: { id: target.path_id ?? target.academy_learning_paths?.id } })) as any;
      ...
      return { pathId: target.path_id ?? target.academy_learning_paths?.id, lessonId: lesson.id, enrollmentId: target.id };
    } catch { return null; }
  };
  ...
  const goNext = async (mode: "next" | "previous" = "next") => {
    ...
    try {
      ...
      navigate({ to: "/app/academy/lesson/$lessonId", params: { lessonId: t.lessonId }, search: { enrollmentId: t.enrollmentId } });
  ...
  const submit = async () => {
    ...
    try {
      ...
      navigate({ to: "/app/academy/lesson/$lessonId", params: { lessonId: t.lessonId }, search: { enrollmentId: t.enrollmentId, q } });
    } finally { setBusy(false); }
  };
  ...
  const chips: Array<{ label: string; mode: "next" | "previous" | "courses" }> = [
    { label: lang === "de" ? "Training fortsetzen" : lang === "ro" ? "Continuă instruirea" : "Continue my training", mode: "next" },
    { label: lang === "de" ? "Heutige Lektion starten" : lang === "ro" ? "Începe lecția de azi" : "Start today's lesson", mode: "next" },
    { label: lang === "de" ? "Letztes Kapitel fortsetzen" : lang === "ro" ? "Reia ultimul capitol" : "Resume last chapter", mode: "next" },
    { label: lang === "de" ? "Vorherige Lektion ansehen" : lang === "ro" ? "Revizuiește lecția anterioară" : "Review previous lesson", mode: "previous" },
    { label: lang === "de" ? "Zum heutigen Thema fragen" : lang === "ro" ? "Întreabă despre subiectul de azi" : "Ask about today's topic", mode: "next" },
  ];
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        <div className="w-full max-w-2xl text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            {lang === "de" ? "Was möchtest du heute lernen?" : lang === "ro" ? "Ce vrei să înveți astăzi?" : "What would you like to learn today?"}
          </p>
          ...
          <div className="mt-6 rounded-2xl border border-border bg-card shadow-lg p-2 focus-within:ring-2 focus-within:ring-primary/30 transition-shadow text-left">
            <Textarea
              ...
              onKeyDown={onKey}
              placeholder={lang === "de" ? "Frag den KI-Trainer…" : lang === "ro" ? "Întreabă-l pe trainerul AI…" : "Ask your AI teacher anything…"}
              rows={2}
            ...
            <div className="flex items-center justify-between px-2 py-1">
              <div className="flex items-center gap-1 text-[11px] text-muted-foreground px-1">
                ...
                {lang === "de" ? "Geerdet in deinem Academy-Lehrplan" : lang === "ro" ? "Bazat pe materialele Academy" : "Grounded in your Academy curriculum"}
              </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2 justify-center">
            {chips.map((c) => (
              <button
                key={c.label}
                onClick={() => c.mode === "courses" ? navigate({ to: "/app/academy/courses" }) : void goNext(c.mode)}
                disabled={busy}
              ...
              >
                {c.mode === "previous" ? <RotateCw className="h-3 w-3" /> : <PlayCircle className="h-3 w-3" />}
                {c.label}
              </button>
            ))}
          </div>
          ...
          {enrollments.length === 0 && (
            <div className="mt-8">
              <Link to="/app/academy/courses" className="inline-flex items-center gap-2 text-sm text-primary hover:underline">
