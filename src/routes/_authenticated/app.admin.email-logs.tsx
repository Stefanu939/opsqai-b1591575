...
function EmailLogsPage() {
  ...
  useEffect(() => {
    ...
    listSuppressedEmails()
      .then((s) => setSuppressed(s as Array<Record<string, unknown>>))
      .catch(() => undefined);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Delivery, suppressions and retries across the entire platform.
            </p>
          </div>
        </div>
            ...
            <tbody>
              {(data?.rows ?? []).map((r) => (
                <tr key={r.message_id} className="border-t">
                  ...
                  <td className="px-4 py-2.5 text-xs text-red-600 dark:text-red-400">
                    {r.error_message ?? ""}
                  </td>
                </tr>
              ))}
