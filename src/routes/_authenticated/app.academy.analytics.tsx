          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {selected.assigned_users} learners · {selected.completion_percent}% completed ·{" "}
                {selected.certificates_issued} certificates
              </div>
            </div>
            {canAssign && (
          ...
          <Card className="overflow-hidden">
            ...
            {cohort === null ? (
              ...
              cohort.map((r) => (
                <div
                  key={r.enrollment_id}
                  className="grid grid-cols-12 gap-2 items-center px-4 py-2.5 border-b last:border-0 text-sm hover:bg-accent/30"
                >
                  <div className="col-span-4 truncate flex items-center gap-2">
                    ...
                    {r.priority === "high" && (
                      <Badge className="text-[10px] bg-orange-500/15 text-orange-600 border-orange-500/30">
                        High
                      </Badge>
                    )}
                  </div>
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Completion, overdue and quiz performance per course. Click a row to open the learner
              cohort.
            </p>
          </div>
          {canAssign && (
            ...
            {courses.map((c) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge
                    variant={c.completion_percent >= 80 ? "default" : "secondary"}
                    className="text-[11px]"
                  >
                ...
                <div className="grid grid-cols-5 gap-3 mt-3 text-[12px]">
                  ...
                  <Stat
                    ...
                    label="Avg quiz"
                    value={c.avg_quiz_score !== null ? `${c.avg_quiz_score}%` : "—"}
                  />
                </div>
              </Card>
            ))}
