  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("audit_log")
        .select("id,user_id,module,action,resource,question,answer_preview,severity,created_at")
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
      ...
      <h1 className="mt-2 text-2xl md:text-3xl font-semibold tracking-tight">
        60 days of operational activity
      </h1>
      <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
        Every mutation, every AI query, every role change — tamper-evident, exportable for audit
        readiness (ISO, SOC 2, GDPR).
      </p>
      ...
      <div className="mt-6 space-y-2">
        {rows.map((r) => {
          ...
          return (
            <Card
              key={r.id}
              className={`p-3 flex items-start gap-3 ${isWarn ? "border-destructive/30" : ""}`}
            >
              <div
                className={`h-8 w-8 rounded-md grid place-items-center shrink-0 ${isWarn ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}
              >
        ...
        {rows.length === 0 && (
          <div className="text-sm text-muted-foreground text-center py-8">Loading audit log…</div>
        )}
      </div>
    </div>
  );
