...
function DemoOverview() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const confs = (msgs.data ?? [])
        .map((m: { confidence: number | null }) => Number(m.confidence))
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
          Enterprise AI Platform · Knowledge Governance · Onboarding · Audit & Compliance — all on
      ...
      <div className="grid gap-3 grid-cols-2 md:grid-cols-5">
        ...
        <Kpi
          label="AI confidence"
          value={`${Math.round(k.avgConfidence * 100)}%`}
          sub="assistant avg."
        />
      </div>
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <Tour
            ...
            step={1}
            title="Ask the AI"
            desc='Try: "What do I do with a damaged pallet on arrival?"'
          />
          <Tour
            to="/demo/app/knowledge"
            icon={BookOpen}
            step={2}
            title="Browse the Knowledge Base"
            desc="See how SOPs, policies, and work instructions are governed."
          />
          <Tour
            to="/demo/app/analytics"
            icon={BarChart3}
            step={3}
            title="Open Analytics"
            desc="AI health, confidence, gaps, and top questions."
          />
          <Tour
            to="/demo/app/audit"
            icon={Activity}
            ...
            desc="60 days of realistic operational activity."
          />
          <Tour
            to="/demo/app/academy"
            icon={GraduationCap}
            ...
            desc="A published onboarding path with 3 chapters."
          />
          <Tour
            to="/demo/app/faq"
            icon={HelpCircle}
            step={6}
            title="Read the FAQ"
            desc="Bilingual quick answers tied to SOPs."
          />
        </div>
      </section>
      ...
      <section className="grid gap-3 md:grid-cols-2">
        <Card className="p-5">
          ...
          <p className="mt-2 text-xs text-muted-foreground leading-relaxed">
            Critical SOPs reviewed within 90 days. Damage-handling coverage is comprehensive.
            Academy completion above target.
          </p>
        </Card>
        <Card className="p-5">
          <div className="text-sm font-semibold">Read-only demo</div>
          <p className="mt-2 text-xs text-muted-foreground leading-relaxed">
            Everything is viewable. Any create / edit / delete / invite / upload / export attempt
            will show you what that action does in your own OPSQAI workspace — with full RBAC and
            audit trail.
          </p>
          <Link
            to="/contact"
