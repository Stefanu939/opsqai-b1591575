...
// SLIDE 2: AGENDA
{
  ...
  const items = [
    ["01", "Operativer Kontext", "Warum Unternehmenswissen der am meisten unterschätzte Vermögenswert ist."],
    ["02", "Die OPSQAI-Lösung", "Einheitliche Plattform: Assistant, Academy, Documents, Governance."],
...
// SLIDE 3: PROBLEM
{
  ...
  subtitle(s, "Operatives Wissen ist fragmentiert in PDFs, Chats und Köpfen. Das Ergebnis: Fehler, Verzögerungen, langsames Onboarding.");
  const stats = [
...
// SLIDE 4: SOLUTION
{
  ...
  subtitle(s, "OPSQAI verwandelt Dokumente, Prozesse und internes Know-how in einen vertrauenswürdigen AI-Assistenten — mit Quellen, Audit und vollständiger Mandantentrennung.", 1.7, true);
  const pillars = [
    ["Quellenbasierte AI", "Antworten ausschließlich aus Ihren Dokumenten. Keine Halluzinationen. Mit Zitaten."],
    ["Multi-Tenant SaaS", "Vollständige Isolation auf Datenbankebene (RLS). Ihre Daten bleiben Ihre Daten."],
...
// SLIDE 5: PRODUCT MAP
{
  ...
  const mods = [
    ["Assistant", "AI-Chat mit Quellen", "Sofortige Antworten aus der Wissensbasis, mit Zitaten und Confidence Score."],
    ["Academy", "Onboarding & Training", "Lernpfade, AI-Lektionen, Quizze und automatische PDF-Zertifikate."],
    ["Enterprise Docs", "Premium-Deliverables", "Berichte, SOPs und Angebote im Stil einer Tier-1-Beratung."],
    ["Governance", "Audit & Kontrolle", "Rollen, Audit-Log, Knowledge Gaps, SOP-Versionierung, Benachrichtigungen."],
  ];
  mods.forEach((m, i) => {
    const x = 0.6 + i * 3.1;
...
// SLIDE 6: ASSISTANT
{
  ...
  const bullets = [
    ["Quellenbasiert", "Jede Antwort basiert ausschließlich auf Ihren Dokumenten — keine Erfindungen."],
    ["Sichtbare Zitate", "Panel mit den exakten Quellen zu jeder Aussage, Direktklick ins Dokument."],
    ...
    ["Knowledge Gaps", "Wenn keine Quelle vorhanden ist, wird eine interne Anfrage für den Manager erstellt."],
  ];
  bullets.forEach((b, i) => {
    const y = 2.2 + i * 0.75;
  ...
  s.addText("Laut SOP-RET-014 §3.2 benötigen B2B-Retouren >500€ die Freigabe des Lagerleiters und eine physische Prüfung an Tor 7. Antwort-SLA: 24h.", {
...
// SLIDE 7: ACADEMY
{
  ...
  subtitle(s, "Von Abteilung → Lernpfad → Kapitel → Lektion → Quiz. Automatisch generierte, öffentlich verifizierbare PDF-Zertifikate.", 1.55);
  const steps = [
    ...
    ["AI-Lektion", "Mit virtuellem Trainer", "Dialogbasierte Erklärungen in der Sprache des Lernenden."],
    ["Quiz & Zertifikat", "Prüfung + PDF", "Öffentlich verifizierbare, von OPSQAI signierte Zertifikate."],
  ];
  steps.forEach((st, i) => {
    const x = 0.6 + i * 3.1;
    card(s, x, 3.2, 2.9, 3.2);
    s.addText(`Schritt ${i + 1}`, { x: x + 0.3, y: 3.35, w: 2, h: 0.3, fontFace: B, fontSize: 10, bold: true, color: TEAL, charSpacing: 2 });
...
// SLIDE 8: ENTERPRISE DOCS
{
  ...
  subtitle(s, "Erzeugen Sie Berichte, SOPs, Angebote und Onboarding-Pakete — mit Cover, KPI-Cards, Tabellen und Export als DOCX / PDF / HTML / MD.", 1.7, true);
  const types = [
    "Executive Summary",
  ...
  s.addText("Professionelles Format: editoriales Cover · Inhaltsverzeichnis · nummerierte Abschnitte · KPI-Cards · Callouts · Zebra-Rows · keine Schusterjungen.", {
...
// SLIDE 9: GOVERNANCE
{
  ...
  const rows = [
    ["RBAC mit 7 Rollen", "Platform Owner → Super Admin → Admin → Manager → Team Leader → Employee → Operator."],
    ["Vollständiges Audit-Log", "Wer, was, wann — für jede Frage, jeden Upload, jede Rollen- oder SOP-Änderung."],
    ["SOP-Versionierung", "Vollständige Historie, Versionsvergleich, Freigaben und Kenntnisnahmen."],
    ["Knowledge Gaps", "Kompletter Zyklus: AI-Ablehnung → interne Anfrage → Manager-Lösung → Übernahme in KB."],
    ["Benachrichtigungen", "Branded E-Mails für Einladungen, Eskalationen, Zertifikate und Sicherheitsmeldungen."],
  ];
  rows.forEach((r, i) => {
    const y = 2.2 + i * 0.85;
...
// SLIDE 10: SECURITY
{
  ...
  subtitle(s, "Multi-Tenant-Architektur mit Isolation auf Datenbankebene. Nichts verlässt die Grenzen Ihres Workspace.", 1.7, true);
  const items = [
...
// SLIDE 11: ARCHITECTURE
{
  ...
  s.addText("Kernprinzipien", { x: 7.75, y: 2.45, w: 4, h: 0.4, fontFace: H, fontSize: 14, bold: true, color: NAVY });
  const principles = [
...
// SLIDE 12: ROI
{
  ...
  s.addText("Was Sie konkret gewinnen", { x: 0.85, y: 4.75, w: 5, h: 0.4, fontFace: H, fontSize: 14, bold: true, color: NAVY });
  const wins = [
...
// SLIDE 14: PRICING
{
  ...
  const plans = [
    { name: "Pilot", price: "30 Tage", desc: "Für die schnelle Validierung", features: ["Bis 25 Nutzer", "1 Workspace", "Assistant + Academy", "E-Mail-Support"], accent: BLUE_L },
    { name: "Standard", price: "Team", desc: "Für operative Teams", features: ["Bis 100 Nutzer", "Alle Module", "SSO + volles RBAC", "Priority Support"], accent: BLUE, highlight: true },
    { name: "Business", price: "Multi-Site", desc: "Für mehrere Standorte", features: ["Bis 500 Nutzer", "Multi-Workspace", "Erweitertes Audit", "SLA 99,9%"], accent: TEAL },
    { name: "Enterprise", price: "Custom", desc: "Für Gruppen & Netzwerke", features: ["Unbegrenzte Nutzer", "Dediziertes Deployment", "AVV & Compliance-Paket", "Dedizierter CSM"], accent: NAVY },
  ];
  plans.forEach((p, i) => {
    const x = 0.6 + i * 3.1;
    const y = 2.2;
    ...
    if (isH) {
      ...
      s.addText("EMPFOHLEN", { x: x + 0.85, y: y - 0.2, w: 1.2, h: 0.35, fontFace: B, fontSize: 8, bold: true, color: WHITE, align: "center", valign: "middle", charSpacing: 2 });
    }
...
// SLIDE 15: PILOT PLAN
{
  ...
  subtitle(s, "Wir führen gemeinsam einen begleiteten Pilot durch: SOP-Ingestion, Academy für eine Abteilung, Impact-Messung.", 1.7, true);
  const phases = [
    ["Woche 1", "Kick-off & Ingestion", "Workspace-Bereitstellung · SOP-Import · Rollenkonfiguration."],
...
// SLIDE 16: CTA
{
  ...
  s.addText("Demo vereinbaren →", { x: 0.6, y: 5.7, w: 3.2, h: 0.7, fontFace: H, fontSize: 14, bold: true, color: WHITE, align: "center", valign: "middle" });
