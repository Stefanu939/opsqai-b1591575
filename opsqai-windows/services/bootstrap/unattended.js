...
const args = [
  ...
  "--install-id",
  a.installId || crypto.randomUUID(),
  "--company",
  a.company?.name || "OPSQAI Customer",
  "--admin-email",
  a.admin.email,
  "--admin-password",
  a.admin.password,
  "--db-mode",
  a.database?.mode || "embedded",
  "--storage-mode",
  a.storage?.mode || "local",
  "--ai",
  JSON.stringify(a.ai || { provider: "none" }),
];
if (a.database?.mode === "external")
  args.push("--db-external", JSON.stringify(a.database.external || {}));
