...
const { loadConfig, saveConfig, programData, programFiles } = require('../../services/common/config');
...
function die(m, code = 1) { console.error(`[migrate] ${m}`); process.exit(code); }
...
const dbContainer  = findContainer('db')       || findContainer('postgres') || findContainer('supabase-db');
const appContainer = findContainer('app')      || findContainer('platform');
if (!dbContainer)  die('could not find the postgres container. Pass its service name via --db-service.');
...
const envMap = Object.fromEntries(dbEnv.map((s) => {
  const i = s.indexOf('=');
...
if (!srcPassword) die('POSTGRES_PASSWORD not found on container — set it in the compose env before migrating.');

log(`pg_dump ${srcDb} -> ${dumpFile}`);
execFileSync('docker', ['exec', '-e', `PGPASSWORD=${srcPassword}`, dbContainer.Name,
  'pg_dump', '-U', srcUser, '-F', 'c', '-f', '/tmp/opsqai-migrate.dump', srcDb], { stdio: 'inherit' });
execFileSync('docker', ['cp', `${dbContainer.Name}:/tmp/opsqai-migrate.dump`, dumpFile], { stdio: 'inherit' });

// -- 3. storage bind mount --------------------------------------------------
const mounts = JSON.parse(runDocker(['inspect', appContainer?.Name || dbContainer.Name]))
  .at(0)?.Mounts || [];
...
if (storageMount && fs.existsSync(storageMount.Source)) {
  ...
  execFileSync('robocopy.exe', [storageMount.Source, storageDest, '/E', '/NFL', '/NDL', '/NP'], { stdio: 'inherit' });
} else {
...
try { cfg = loadConfig(); }
  ...
  cfg = {
    ...
    company: { name: envMap.OPSQAI_COMPANY_NAME || 'Migrated from Docker', contactEmail: envMap.OPSQAI_ADMIN_EMAIL || '', timezone: 'UTC' },
...
saveConfig(cfg);
fs.writeFileSync(path.join(importDir, 'source.json'), JSON.stringify({
  container: dbContainer.Name, srcUser, srcDb, at: new Date().toISOString(),
}, null, 2));
...
['OpsqaiUpdater', 'OpsqaiCaddy', 'OpsqaiWorker', 'OpsqaiPlatform', 'OpsqaiDatabase']
  .forEach((s) => winsw(s, 'stop'));
...
(async function main() {
  ...
  execFileSync(pgRestore, ['-h', '127.0.0.1', '-p', String(cfg.database.embedded.port),
    '-U', 'opsqai', '-d', 'opsqai', '--clean', '--if-exists', dumpFile],
    { stdio: 'inherit', env: { ...process.env, PGPASSWORD: newPgPassword } });

  ['OpsqaiPlatform', 'OpsqaiWorker', 'OpsqaiCaddy', 'OpsqaiUpdater'].forEach((s) => winsw(s, 'start'));

  log('probing https://localhost/health');
  const probe = await new Promise((resolve) => {
    const req = https.get('https://localhost/health', { rejectUnauthorized: false, timeout: 15000 }, (res) => {
  ...
  if (has('stop-containers')) {
    ...
    spawnSync('docker', ['compose', '-f', composePath, 'down'], { stdio: 'inherit', cwd: composeDir });
  } else {
