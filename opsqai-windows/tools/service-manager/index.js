...
const SERVICES = ['OpsqaiDatabase', 'OpsqaiPlatform', 'OpsqaiWorker', 'OpsqaiCaddy', 'OpsqaiUpdater'];
...
const cmds = {
  ...
  start(name)   { for (const s of name ? [name] : SERVICES) console.log(s, svc(s, 'start').code === 0 ? 'ok' : 'fail'); },
  stop(name)    { for (const s of name ? [name] : [...SERVICES].reverse()) console.log(s, svc(s, 'stop').code === 0 ? 'ok' : 'fail'); },
  restart(name) { cmds.stop(name); cmds.start(name); },
  ...
  async health() {
    const r = await new Promise((resolve) => {
      https.get('https://localhost/health', { rejectUnauthorized: false, timeout: 5000 }, (res) => {
        res.resume();
        resolve({ status: res.statusCode });
      }).on('error', (e) => resolve({ status: 0, error: e.message }))
        .on('timeout', function () { this.destroy(); resolve({ status: 0, error: 'timeout' }); });
    });
  ...
  update(sub = 'check') {
    ...
    if (sub === 'apply') {
      ...
      if (!isAdmin()) die('update apply requires administrator (right-click Command Prompt → Run as administrator).');
  ...
  backup(dest) {
    ...
    if (cfg.database.mode === 'embedded') {
      ...
      execFileSync(pgDump, ['-h', '127.0.0.1', '-p', port, '-U', 'opsqai', '-F', 'c', '-f', dumpFile, 'opsqai'],
        { stdio: 'inherit', env: { ...process.env, PGPASSWORD: pw } });
    } else {
      const ext = cfg.database.external;
      const pgDump = programFiles('pgsql', 'bin', 'pg_dump.exe');
      execFileSync(pgDump, ['-h', ext.host, '-p', String(ext.port), '-U', ext.username, '-F', 'c', '-f', dumpFile, ext.database],
    ...
    if (cfg.storage.mode === 'local' && fs.existsSync(cfg.storage.local.path)) {
      ...
      execFileSync('robocopy.exe', [cfg.storage.local.path, storageDest, '/MIR', '/NFL', '/NDL', '/NP'], { stdio: 'inherit' });
