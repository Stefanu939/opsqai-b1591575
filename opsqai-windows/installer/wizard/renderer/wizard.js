...
$$('input[name="db-mode"]').forEach((r) => r.addEventListener('change', () => {
  $('#db-external').hidden = r.value !== 'external' || !r.checked;
}));
$$('input[name="storage-mode"]').forEach((r) => r.addEventListener('change', () => {
...
function render() {
  ...
  if (state.step === 9)       { nextBtn.textContent = 'Install'; nextBtn.classList.add('btn-primary'); }
  else if (state.step === 10) { nextBtn.textContent = 'Finish';  nextBtn.disabled = true; }
...
async function validate() {
  switch (state.step) {
    case 2:
      if (!$('#license-accept').checked) { alert('You must accept the license agreement to continue.'); return false; }
      state.data.licenseAccepted = true; return true;
    case 4: {
      const name = $('#company-name').value.trim();
      const email = $('#company-email').value.trim();
      if (!name)                    { alert('Enter your company name.'); return false; }
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) { alert('Enter a valid contact email.'); return false; }
      state.data.company = { name, contactEmail: email, timezone: $('#company-tz').value };
      return true;
    }
    case 5: {
      const email = $('#admin-email').value.trim();
      const p1 = $('#admin-password').value, p2 = $('#admin-password2').value;
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) { alert('Enter a valid admin email.'); return false; }
      if (p1.length < 12)           { alert('Admin password must be at least 12 characters.'); return false; }
    ...
    case 6: {
      ...
      if (mode === 'external') {
        ...
        if (!host || !dbName || !user || !pass) { alert('Fill in all external database fields.'); return false; }
        state.data.database = { mode: 'external', external: { host, port: Number(port), database: dbName, username: user, password: pass } };
      } else {
    ...
    case 7: {
      ...
      if (mode === 's3') {
        ...
        if (!endpoint || !bucket || !key || !secret) { alert('Fill in all S3 fields.'); return false; }
        state.data.storage = { mode: 's3', s3: { endpoint, region: $('#s3-region').value.trim(), bucket, accessKey: key, secretKey: secret } };
      } else {
    ...
    case 8: {
      ...
      state.data.ai = provider === 'none' ? { provider: 'none' } : { provider, apiKey: key || null };
      return true;
    }
    default: return true;
  }
...
function renderReview() {
  ...
  const rows = [
    ...
    ['Password', '••••••••'],
    ['section', 'Database'],
    ['Mode', d.database?.mode === 'embedded' ? 'Embedded PostgreSQL 16' : `External (${d.database?.external?.host}:${d.database?.external?.port})`],
    ['section', 'Storage'],
...
function setBar(pct) { $('#progress-bar').style.width = pct + '%'; }
