...
const installRoot = path.resolve(app.isPackaged
...
function createWindow() {
  ...
  win.on('close', (e) => {
    if (win.__installing) { e.preventDefault(); return; }
  });
}
...
ipcMain.handle('wizard:cancel', async () => {
  const r = await dialog.showMessageBox(win, {
    type: 'warning', buttons: ['Continue setup', 'Exit installer'], defaultId: 0, cancelId: 0,
...
ipcMain.handle('wizard:validatePort', async (_e, port) => {
  const p = Number(port);
  if (!Number.isInteger(p) || p < 1024 || p > 65535) return { ok: false, error: 'Port must be 1024–65535' };
  return { ok: true };
});
...
ipcMain.handle('wizard:install', async (event, config) => {
  ...
  const args = [
    ...
    '--db-mode',         config.database.mode,
    '--storage-mode',    config.storage.mode,
    '--ai',              JSON.stringify(config.ai),
  ];
